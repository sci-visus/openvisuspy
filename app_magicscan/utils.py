...
# ////////////////////////////////////////////////////////
def bbox_corners(bbox_dict):
    """
    Parameters
    ----------
    bbox_dict : dict with keys x,y,width,height from ColumnDataSource
                where x,y are Bokeh's *centre* of the rectangle.

    Returns
    -------
    (x0, y0, x1, y1)  # lower‑left and upper‑right (inclusive) in *image* coords
    """
    cx, cy   = bbox_dict["x"][0], bbox_dict["y"][0]
    w,  h    = bbox_dict["width"][0], bbox_dict["height"][0]
    x0, y0   = cx - w/2,  cy - h/2
    x1, y1   = cx + w/2,  cy + h/2
