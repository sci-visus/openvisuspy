...
class Canvas:
	...
	def createFigure(self):
		...
		if self.view_choice  == "SYNC_VIEW": # sync_view bokeh options
			...
			self.fig=bokeh.plotting.figure(tools=[self.pan_tool,self.wheel_zoom_tool])
		...
		# 2d image (eventually multichannel)
			...
			if all([
				...
				if img.dtype == np.uint32:    
					...
					toggle_code = """
					if (toolbar.active_drag === pan_tool) {
						toolbar.active_drag = freehand_tool;
						console.log("Double-click: Switched to freehand drawing tool");
					} else {
						toolbar.active_drag = pan_tool;
						console.log("Double-click: Switched to pan tool");
					}
					"""
					self.fig.js_on_event("doubletap", bokeh.models.CustomJS(
						args=dict(pan_tool=self.pan_tool, freehand_tool=freehand_tool, toolbar=self.fig.toolbar),
						code=toggle_code
					))
